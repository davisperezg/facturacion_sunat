...
class WS_le_compras8_1_detalles extends CI_Controller {
    ...
    public function ws_select(){        
        ...
        $data = $this->le_compras8_1_detalles_model->select(3, '', array('periodo' => $anio.$mes."00", ' ORDER BY id DESC'));       
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
    }
    
    public function ws_select_item(){
        $data = $this->le_compras8_1_detalles_model->select(2,'',array('id' => $this->uri->segment(3)));
        echo json_encode($data, JSON_UNESCAPED_UNICODE);
    }
