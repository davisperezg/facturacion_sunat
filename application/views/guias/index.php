...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $('#btn_buscar_entidad').on('click', function(){
            ...
            var ruta_url = base_url + 'index.php/WS_guias/ws_select/' + pagina + '/' + filas_por_pagina + '/' + param_entidad_id + '/' + param_serie + '/' + param_numero + '/' + param_fecha_emision_desde + '/' + param_fecha_emision_hasta;
            ...
            $.getJSON(ruta_url)
                .done(function (data) {
                    ...
                    (data.guias).forEach(function (repo) {
                        agregarFila(numero_orden, repo.guia_id, repo.entidad, repo.fecha_emision, repo.fecha_traslado, repo.serie, repo.numero, repo.venta_numero, repo.estado_operacion);
                        numero_orden ++;
                    });
            });
        });
        ...
        $('#div_contenedor').on('click', '.pajaro', function(){
            ...
            var url_l = base_url + 'index.php/WS_guias/ws_select/' + pagina + '/' + filas_por_pagina + '/' + param_entidad_id + '/' + param_serie + '/' + param_numero + '/' + param_fecha_emision_desde + '/' + param_fecha_emision_hasta;
            $.getJSON(url_l)
            .done(function (data) {
        ...
        $("#tabla_guia_id").on('click', '.btn_perfil_guia', function(){
            ...
            ruta_url = base_url + 'index.php/guias/modal_detalle/';
            $("#myModal").load(ruta_url);
        });        
        
        $("#nueva_guia").attr('href', base_url+'index.php/guias/operacion');        
        //Modificar        
        $('#tabla_guia_id').on('click', '.btn_editar', function(){
            let guia_id = $(this).attr('id');
            let url_l = base_url + 'index.php/guias/operacion/#' + guia_id;
        ...
        $('#tabla_guia_id').on('click', '.API_SUNAT---', function(){
            ...
            let url_xml = base_url + "index.php/guias/enviarSunat/" + guia_id;
            console.log(url_xml);
            $.getJSON(url_xml)            
            .done(function(data){        
        ...
        $('#tabla_guia_id').on('click', '.API_SUNAT', function(){   
            var guia_id = $(this).attr('id');
            let ruta = base_url + "index.php/WS_guias/api_guias/" + guia_id;
            $.getJSON(ruta)
            .done(function(data){
    ...
    function carga_inicial(){
        //CARGA INICIAL
        var url_l = base_url + 'index.php/WS_guias/ws_select/' + pagina_inicial + '/' + filas_por_pagina + '/' + param_stand_url + '/' + param_stand_url + '/' + param_stand_url + '/' + param_stand_url + '/' + param_stand_url;
        console.log(url_l);
        $.getJSON(url_l)
            .done(function (data) {
   ...
   var nepele;
    function agregarFila(numero_orden, guia_id, entidad, fecha_emision, fecha_traslado, serie, numero, venta_numero, estado_operacion){            
        ...
        fila += '<td align="center"><a id="'+guia_id+'" class="btn btn-default btn-xs btn_perfil_guia" data-toggle="modal" data-target="#myModal">'+numero_orden+'</a></td>';
        fila += '<td>'+entidad+'</td>';
        fila += '<td>'+fecha_emision+'</td>';
        fila += '<td>'+fecha_traslado+'</td>';
        fila += '<td>'+serie+'-'+numero+'</td>';
        fila += '<td>'+venta_numero+'</td>';
        fila += '<td align="center"><a target="_blank" href="'+base_url+'index.php/guias/pdf_a4/'+guia_id+'"><img title="Ver Pdf" src="'+base_url+'images/pdf.png"></a></td>';        
